...
public class SyncMessageOne extends Service {

    private static final String TAG = SyncMessageOne.class.getName();
    Active active;
    ...
    private List<MessageModel> getSent() {
        ...
        while (c.moveToNext()) {
            ...
            if(active.getValue(Admin.OLD_MESSAGE_UPDATE).equalsIgnoreCase(model.getDate())){
                break;
            }
            messageModels.add(model);
        }
